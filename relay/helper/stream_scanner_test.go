...
func TestStreamScannerHandler_PerCallClientCancelDrainsUpstream(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	...
	go func() {
		StreamScannerHandler(c, resp, info, func(data string, sr *StreamResult) {
			count.Add(1)
			if writeErr := StringData(c, data); writeErr != nil {
				sr.Stop(writeErr)
			}
			if data == "first" {
				close(firstHandled)
	...
	cancel()
	_, err = fmt.Fprint(pw, "data: second\ndata: [DONE]\n")
	require.NoError(t, err, "per-call upstream must remain readable after client disconnect")

	select {
	case <-done:
	case <-time.After(2 * time.Second):
		t.Fatal("handler did not finish after draining per-call upstream")
	}
