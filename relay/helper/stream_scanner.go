...
const streamDiscardClientWritesKey = "stream_discard_client_writes"

type streamClientState struct {
	cancelUpstreamOnGone bool
	discardWrites        atomic.Bool
}

func shouldDiscardStreamWrite(c *gin.Context) bool {
	if c == nil {
		return false
	}
	value, exists := c.Get(streamDiscardClientWritesKey)
	if !exists {
		return false
	}
	state, ok := value.(*streamClientState)
	if !ok {
		return false
	}
	if state.discardWrites.Load() {
		return true
	}
	if !state.cancelUpstreamOnGone && requestContextDone(c) {
		state.discardWrites.Store(true)
		return true
	}
	return false
}
...
func StreamScannerHandler(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo, dataHandler func(data string, sr *StreamResult)) {
	...
	cancelUpstreamOnClientGone := info.ShouldCancelUpstreamOnClientGone()
	clientState := &streamClientState{cancelUpstreamOnGone: cancelUpstreamOnClientGone}
	c.Set(streamDiscardClientWritesKey, clientState)
	...
	// Token-priced streams stop upstream generation when the client leaves.
	// Per-call streams keep draining and parsing the upstream response for
	// accurate billing, but silently discard writes to the disconnected client.
	...
	for {
		...
		case <-clientDone:
			clientState.discardWrites.Store(true)
			if cancelUpstreamOnClientGone {
